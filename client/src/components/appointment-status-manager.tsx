...
interface AppointmentStatusManagerProps {
  appointment: Partial<Appointment>; // ✅ Allow partial props
  userRole?: "doctor" | "patient" | "admin"; // ✅ Optional for notification dashboard
  onStatusChange?: () => void;
}
...
export default function AppointmentStatusManager({
  ...
  const [selectedStatus, setSelectedStatus] = useState<
    "scheduled" | "completed" | "cancelled" | "no-show" | "awaiting_payment" | "confirmed"
  ...
  const StatusIcon = statusConfig[appointment.status || "scheduled"].icon;
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {(["scheduled", "completed", "cancelled", "no-show"] as const).map(
                  (status) => (
                    <button
                      key={status}
