...
      // FIX: Use storage method for single-day query. Storage handles the internal conversion.
      availability = await storage.getDoctorAvailability(doctorId, isoDayOfWeek);
      
      // FIX: Assuming storage returns objects with dayOfWeek in JS format (0-6), convert them back to ISO for consistency.
...
    } else {
      // All-days query 
      // FIX: Use the assumed getAllDoctorAvailability method (as getDoctorAvailability is designed for single-day)
      const result = await (storage as any).getAllDoctorAvailability(doctorId);
      
      // Convert all JS days (stored) to ISO for response
      availability = result.map((slot: any) => ({
        ...slot.toObject(),
        dayOfWeek: convertJsDayToIso(slot.dayOfWeek)
      }));
    }
...
        
        // CORRECTED CALL: Pass the ISO day (1-7) to match the interface contract
