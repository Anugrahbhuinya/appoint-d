...
// ==========================================
// APPOINTMENT SCHEMA (FIXED ENUMS)
// ==========================================
...
  // ✅ FIX: Added 'pending' status
  status: 'pending' | 'scheduled' | 'completed' | 'cancelled' | 'no-show' | 'awaiting_payment' | 'confirmed';
  consultationFee: number;
  notes?: string;
  prescription?: string;
...
  // ✅ FIX: Added 'pending' to the Mongoose enum list
  status: { type: String, default: 'scheduled', enum: ['pending', 'scheduled', 'completed', 'cancelled', 'no-show', 'awaiting_payment', 'confirmed'] },
  consultationFee: { type: Number, required: true },
  notes: { type: String },
...
  status: z.enum(['pending', 'scheduled', 'completed', 'cancelled', 'no-show', 'awaiting_payment', 'confirmed']).default('pending'),
  consultationFee: z.number().min(0),
